...
function SkeletonBar() {
  return <div className="h-full bg-gradient-to-b from-slate-200 to-slate-100 rounded-t-lg animate-pulse" />;
}
...
// ── Stat Card ─────────────────────────────────────────────────────────────────
function StatCard({ label, value, trend, trendUp, trendColor, bg, sparkBars, activeColor, inactiveColor, isLoading }) {
  return (
    <div className={`${bg} p-8 rounded-xl`} style={{ boxShadow: "0 40px 40px -20px rgba(25,28,30,0.06)" }}>
      ...
      <Sparkline bars={sparkBars} activeColor={activeColor} inactiveColor={inactiveColor} isLoading={isLoading} />
    </div>
  );
}
...
function KeyMetricsCards({ netBalance, profitMargin, savingsRate, isLoading }) {
  ...
  return (
    <div className="col-span-2 grid grid-cols-3 gap-4">
      {metrics.map((metric) => (
        <div key={metric.label} className={`${metric.bgColor} p-6 rounded-xl border border-slate-200`} style={{ boxShadow: "0 40px 40px -20px rgba(25,28,30,0.06)" }}>
          {isLoading ? (
            <><SkeletonLine /><div className="mt-4 h-8 w-24 bg-slate-200 rounded animate-pulse" /></>
          ) : (
            <>
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] uppercase tracking-[0.1em] font-bold text-slate-400">{metric.label}</p>
                <Icon name={metric.icon} className={`text-xl ${metric.color}`} filled />
              </div>
...
function TopExpenseCategories({ categoryBreakdown, totalExpense, isLoading }) {
  const colors = ["#00426d", "#005a92", "#006a6a", "#9acbff", "#c9e4ff"];
  const topCategories = categoryBreakdown
    ? Object.entries(categoryBreakdown).sort(([, a], [, b]) => b - a).slice(0, 5)
    : [];

  return (
    <div className="col-span-2 bg-white p-8 rounded-xl" style={{ boxShadow: "0 40px 40px -20px rgba(25,28,30,0.06)" }}>
      <h3 className="text-lg font-bold text-[#191c1e] mb-8" style={{ fontFamily: "Manrope, sans-serif" }}>Top Expense Categories</h3>
      <div className="space-y-4">
        {isLoading ? (
          [...Array(5)].map((_, i) => (
            <div key={i}><SkeletonLine /><div className="h-2 w-full bg-[#eceef0] rounded-full mt-2" /></div>
          ))
            ...
            return (
              <div key={cat}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-bold text-[#00426d]">{pct}% · ₹{amt.toLocaleString("en-IN")}</span>
                </div>
...
// ── Category Breakdown ────────────────────────────────────────────────────────
function CategoryBreakdown({ categoryBreakdown, totalExpense, isLoading }) {
  if (isLoading) return (
    <div className="bg-white p-8 rounded-xl" style={{ boxShadow: "0 40px 40px -20px rgba(25,28,30,0.06)" }}>
      <h3 className="text-sm font-bold text-[#00426d] mb-6">Complete Expense Breakdown</h3>
      <div className="space-y-4">{[...Array(4)].map((_, i) => <div key={i}><SkeletonLine /><div className="h-2 w-full bg-[#eceef0] rounded-full mt-2" /></div>)}</div>
    </div>
  );
...
// ── AI Insights ───────────────────────────────────────────────────────────────
function AIInsights({ summaryText, recommendation, financialHealth, profitMargin, anomalies, isLoading }) {
  const healthColor = financialHealth === "good" ? "text-green-600" : financialHealth === "average" ? "text-yellow-600" : "text-red-500";
  return (
    <div className="bg-[#8cf3f3]/30 backdrop-blur-xl p-8 rounded-xl border border-white/20">
      ...
      <ul className="space-y-6">
        {isLoading ? (
          ["Business Summary", "Financial Health", "Recommendation"].map((label) => (
            <li key={label} className="flex gap-4">
              <span className="w-1 h-12 bg-slate-200 rounded-full block flex-shrink-0" />
              <div className="flex-1"><p className="text-sm font-semibold text-[#191c1e] mb-2">{label}</p><SkeletonLine /></div>
            </li>
          ))
        ) : (
          <>
            <li className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{summaryText || `Profit Margin: ${profitMargin ?? "—"}%`}</p>
              </div>
            </li>
            <li className="flex gap-4">
            ...
            <li className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{recommendation || "No recommendation available."}</p>
              </div>
            </li>
            {anomalies?.length > 0 && (
              <li className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">{anomalies.length} unusual transaction(s) flagged by AI</p>
                </div>
              </li>
            )}
          </>
...
// ── Recent Activity ───────────────────────────────────────────────────────────
function RecentActivity({ totalTransactions, anomalyCount, isLoading }) {
  return (
    <div className="bg-white p-8 rounded-xl" style={{ boxShadow: "0 40px 40px -20px rgba(25,28,30,0.06)" }}>
      <h3 className="text-lg font-bold text-[#191c1e] mb-6" style={{ fontFamily: "Manrope, sans-serif" }}>Recent Activity</h3>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-bold text-[#191c1e]">Total Transactions</p>
            <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Imported from overview API</p>
          </div>
          <span className="text-lg font-bold text-[#00426d]">
            {isLoading ? <div className="w-12 h-6 bg-slate-200 rounded animate-pulse" /> : totalTransactions ?? "—"}
          </span>
        </div>
        <div className="flex items-center justify-between border-t border-gray-100 pt-4">
          <div>
            <p className="text-sm font-bold text-[#191c1e]">Total Anomalies</p>
            <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Detected by AI</p>
          </div>
          <span className="text-lg font-bold text-[#00426d]">
            {isLoading ? <div className="w-12 h-6 bg-slate-200 rounded animate-pulse" /> : anomalyCount ?? 0}
          </span>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-screen bg-[#f7f9fb] text-[#191c1e]" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <main className="md:ml-64 flex-1 p-8 lg:p-12 min-h-screen">
        {/* Mobile menu button */}
        <button
          className="md:hidden mb-6 p-2 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors"
        ...
        <header className="flex justify-between items-end mb-10">
          <div>
            <span className="uppercase tracking-[0.2em] text-[10px] font-bold text-slate-400 mb-2 block">Executive Summary</span>
